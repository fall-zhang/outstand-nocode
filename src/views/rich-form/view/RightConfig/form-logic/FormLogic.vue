...
<template>
  <el-drawer destroy-on-close size="60%" :modal="false" append-to-body :close-on-press-escape="false"
    ...
    <el-tabs v-model="activeTab" class="demo-tabs">
      <el-tab-pane v-for="tab in tabs" :label="tab.label" :name="tab.value" :key="tab.value">
        <el-empty v-if="!tab.rules.length">
          <el-button type="primary" icon="plus" @click="handleAction('add')">{{ t('rf.public.add') }}</el-button>
        </el-empty>
    ...
    <el-button v-show="tabs[curIndex].rules.length" class="button" @click="handleAction('add')">
      {{ t('rf.public.add') }}
    </el-button>
    <template #footer>
